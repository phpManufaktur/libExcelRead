...
class CompoundDocument {
     ...
     * This is an array of the SecIDs of all sectors used by
     ...
     * Contains SecIDs of all sectors used by user streams
     * 
     * @var array
     */
    public $sat = array();
     ...
     * Contains SecIDs of all short streams
     * 
     * @var array
     */
    public $ssat = array();
     ...
     * 'type' => <type of the entry, 0 = empty, 1 = storage, 2 = stream, 5 =
     * root storage>,
     ...
     * 'rootDirId' => <DirID of the root node, if this entry is a storage, -1
     * otherwise>,
    ...
    private function _buildMsat ($offset, $size, $first = false) {
        ...
        $next = $first ? $this->header['msatSecId'] : $this->_value(
                $offset + $size - 4);
        if ($next == -2)
            return;
    ...
    private function _getShortStream ($ssecId) {
        ...
        for ($sector = $ssecId; $sector >= 0; $sector = $this->ssat[$sector])
            $result .= substr($this->_shortData, 
                    $this->header['ssecSize'] * $sector, 
                    $this->header['ssecSize']);
        return $result;
    }
    ...
    private function _getNormalStream ($secId) {
        ...
        for ($sector = $secId; $sector >= 0; $sector = $this->sat[$sector])
            $result .= substr($this->_data, $this->sectorOffset($sector), 
                    $this->header['secSize']);
        return $result;
    }

    private function _getStream ($id) {
        return $this->directory[$id]['desc']['size'] <
                 $this->header['minStreamSize'] ? $this->_getShortStream(
                        $this->directory[$id]['desc']['secId']) : $this->_getNormalStream(
                        $this->directory[$id]['desc']['secId']);
            }
                ...
                for ($sector = $this->header['dirSecId']; $sector >= 0; $sector = $this->sat[$sector]) {
                    ...
                    for ($i = 0; $i < $dirCount; $i++, $id++) {
                        $desc = @unpack(
                                ...
                                substr($this->_data, $sectorOffset + $i * 0x80, 
                                        0x80));
                        if ($desc['type'] == 0)
                            break;
                        if ($desc['type'] == 5) {
                            $this->header['sstreamSecId'] = $desc['secId'];
                            $this->_shortData = $this->_getNormalStream(
                                    $desc['secId']);
                        }
                        $desc['name'] = iconv('utf-16le', $this->charset, 
                                substr($desc['name'], 0, 
                                        $desc['nameLength'] - 2));
                        $this->directory[$id]['desc'] = $desc;
                        if ($desc['type'] == 2)
                            $this->directory[$id]['data'] = $this->_getStream(
                                    $id);
                    }
                }
            }
