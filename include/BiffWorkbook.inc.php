...
class BiffWorkbook
{
	...
	private function _xfExtColor ($subOffset)
	{
		...
		switch ($data ['type'])
		{
			...
			case 3:
			    // fixed: changed $self::_setBrightness() to self::_setBrightness - @author
	...
	private function _onFont ($record)
	{
		...
		$this->fonts [] = new BiffWorkbookFont ($this, $raw);
	}
	...
	private function _onPalette ($record)
	{
		...
		for ($i = 0; $i < $colors; $i ++)
			...
			//$this->palette [$i] = $this->_value (2 + $i * 4, 3, 'H6');
	...
	private function _onFormula ($record)
	{
		...
		else
		{
			...
			if ($style->formatType == BiffWorkbookStyle::ftDate)
			{
				list ($value, $rawValue) = $this->_getDate ($style, $value);
				$type = BiffWorkbookCell::typeDate;
			}
			else 
			{
				$value = $rawValue = $this->_value (6, 8, 'd');
	...
	private function _onNumber ($record)
	{
		...
		$style = $this->styles [$raw ['xf']];
		if ($style->formatType == BiffWorkbookStyle::ftDate)
			list ($value, $rawValue) = $this->_getDate ($raw ['xf'], $value);
		else
		{
